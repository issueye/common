...
func InitLogger(cfg *Config) (*zap.SugaredLogger, *zap.Logger) {
	infoLogFileName := filepath.Join(cfg.Path, "info", "logger.log")
	errorLogFileName := filepath.Join(cfg.Path, "error", "logger.log")
	...
	Log := logger.Sugar()
	Log.Debugf("【初始化zap日志完成!】【%s】", time.Now().Format(utils.FormatDateTimeMs))
	return Log, logger
}
